...
class FormatError extends PyError {
  ...
  static notAllowed(outrage, forType, code = '\0') {
    ...
    let msg = outrage + " not allowed " + withOrIn + forType + " format specifier" + codeAsString;
...
class KeyError extends PyError {
  constructor(message) {
...
class InternalFormat {
  ...
  static Formatter = class {
    ...
    groupDigits(groupSize, comma) {
      ...
      if (commasNeeded > 0) {
        ...
        let from = this.start + this.lenSign + this.lenWhole; // Open a space into which the whole part will expand.

        this.makeSpaceAt(from, commasNeeded); // Index *just after* the end of that space.

        let to = from + commasNeeded; // The whole part will be longer by the number of commas to be inserted.
    ...
    _pad2(leftIndex, n) {
      ...
      let fill = this.spec.getFill(' '); // Start by assuming padding is all leading ('>' case or '=')

      let leading = n; // Split the total padding according to the alignment
      ...
      let trailing = n - leading; // Insert the leading space
    ...
    zeroPadAfterSignWithGroupingFixup(groupSize, comma) {
      ...
      let firstZero = this.start + this.lenSign; // One beyond last digit of the whole-part.

      let p = firstZero + this.lenWhole; // Step back down the result array visiting the commas. (Easiest to do all of them.)
  ...
  static Parser = class {
    ...
    parse() {
      ...
      let precision = InternalFormat.Spec.UNSPECIFIED; // Scan [[fill]align] ...
...
class MarkupIterator {
  static TYPE = 'MarkupIterator'; // switcher to emulate overloaded constructor
  ...
  nextChunk() {
    ...
    let result = new MarkupIterator.Chunk(); // pos = index is the index of the first text not already chunked

    let pos = this.index; // Advance pos to the first '{' that is not a "{{" (escaped brace), or pos<0 if none such.
...
class PyType {
  ...
  getFieldObject(fieldName, bytes, lookups) {
    ...
    let obj = null; // the value can be undefined/null, so need to use hasOwnProperty to explicitly see if key exists
...
class PyString extends PyType {
  ...
  __format__(formatSpec) {
    ...
    let spec = InternalFormat.fromText(formatSpec); // Get a formatter for the specification
...
class FloatFormatter extends InternalFormat.Formatter {
  ...
  format(value, positivePrefix = undefined) {
    ...
    let precision = this.spec.getPrecision(InternalFormat.Spec.specified(this.spec.type) ? 6 : 12); // Guard against excessive result precision
  ...
  appendExponent(exp) {
    ...
    let e; // Deal with sign and leading-zero convention by explicit tests.
...
class PyInteger extends PyType {
  ...
  __format__(formatSpec) {
    // Parse the specification
    let spec = InternalFormat.fromText(formatSpec); // Try to make an integer formatter from the specification
